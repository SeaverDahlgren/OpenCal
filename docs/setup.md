...
- Run `npm run deploy:check` before staging/production deploys. It summarizes hard errors and softer warnings from the current env. Today it will still block a true production rollout because the `postgres` and `redis` runtime adapters are not implemented yet.
...
- The OAuth `state` payload is now signed and short-lived. If a callback arrives with a stale or tampered state token, OpenCal drops the embedded return target instead of trusting it.
- Return targets are prefix-allowlisted too. Defaults:
  - `opencal://`
  - `exp://`
  - `exps://`
...
- Mobile clients should also send `x-opencal-app-version`. If available, `x-opencal-platform` is recorded too so support can inspect the last seen client build and platform on a session.
